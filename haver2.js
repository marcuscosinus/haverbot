...
const mongoUrl = 'mongodb://marcus:haverbot@ds021000.mlab.com:21000/haver';
...
client.on('message', msg => {
  ...
  if (err) {
    ...
    members.forEach( (v,k) => {
    collection.update( { '_id' : { $exists : true} } , { _id: v.user.id, name: v.user.username }, { upsert: true }, function (err, result) {
      if (err) {
        console.log(err);
      } else {
        console.log('Inserted :', v.user.username);
      }
      //Close connection
      db.close();
    });
